...
이에 따라, vector space의 tensor product의 성질에 의해 각각의 Hilbert space의 state를 $$ \ket{\psi_{i}} ∈ \mathcal{H_{i}} $$ 라고 했을 때, composite system의 state $$ \ket{\psi} ∈ \mathcal{H_{1}} × \mathcal{H_{2}} × \cdots × \mathcal{H_{n}} $$는 다음과 같이 표현된다.
...
> **Entangled state**<br>
> pure state $$ \ket{\psi} ∈ \mathcal{H_{A}} × \mathcal{H_{B}} $$ 가 $$ \ket{\psi_{A}} ∈ \mathcal{H_{A}} $$ 와  $$ \ket{\psi_{B}} ∈ \mathcal{H_{B}} $$ 인 어느 state의 product state $$ \ket{\psi_{A}} \otimes \ket{\psi_{B}} $$ 로 표현될 수 없으면 entangled 되었다고 한다.
...
> pure state $$ \ket{\psi} ∈ \mathcal{H_{A}} × \mathcal{H_{B}} $$에 대해 A system의 orthonormal basis를 $$ \ket{j} $$, B system의 orthonormal basis를 $$ \ket{k} $$ 라고 하면, complex matrix의 성분 $$ a_{jk} $$ 에 대해, 다음과 같이 분해된다.
...
$$ \ket{i_{A}} $$ 와 $$ \ket{i_{B}} $$ 는 **Schmidt base** 이며, $$ \lambda_{i} $$ 는 **Schimidt coefficient** 이고, Schimidt coefficient의 개수가 **Schmidt number** $$ d $$이다.
